        ...
        <div className="mx-auto max-w-2xl lg:max-w-none">
          ...
          <div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8">
            ...
            <Tab.Group as="div" className="flex flex-col-reverse">
              {/* Image selector */}
              <div className="mx-auto mt-6 w-full max-w-2xl p-2 lg:max-w-none">
            ...
            <div className="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
              <h1 className="text-2xl font-bold tracking-tight text-gray-900 md:text-3xl">
                {roomDetails?.hostelName.replace(/_/g, " ")}
              </h1>

              <div className="flex gap-4 pt-4">
                <h2 className="rounded-xl bg-gray-200 p-2 text-lg font-semibold text-gray-700">
                  {roomDetails?.floor?.replace(/_/g, " ")}
                </h2>
                <h2
                  className={`rounded-xl p-2 text-lg font-semibold text-gray-100 ${roomDetails?.bookingStatus === "CANCELED" ? "bg-green-500" : "bg-red-400"}`}
                >
                  {roomDetails?.bookingStatus === "CANCELED"
                    ? " Available"
                    : "Not Available"}
                </h2>
              ...
              <div className="mt-4">
                ...
                <ul className="mt-2 space-y-2">
                  <li className="flex items-baseline gap-2">
                    {" "}
                    <FaHotTub /> Geaser: {roomDetails?.geaser}
                  </li>
                  <li className="flex items-baseline gap-2">
                    {" "}
                    <FaHotTub />
                  ...
                  <li className="flex items-baseline gap-2">
                    <FaHotTub /> Max Adult Allowed: {roomDetails?.maxAdult}
                  </li>
                  <li className="flex items-baseline gap-2">
                    <FaHotTub /> Max Child Allowed: {roomDetails?.maxChild}
                  </li>
                  <li className="flex items-baseline gap-2">
                    <FaHotTub /> Occupancy: {roomDetails?.occupancy}
                  </li>
                </ul>
              </div>
              ...
              <section aria-labelledby="details-heading" className="mt-12">
                ...
                <Accordion type="single" collapsible className="mt-4 w-full">
                  <AccordionItem value="item-2">
                    ...
                    {roomFeatures[roomDetails.hostelName].features.map(
                      (d, index) => (
                        <AccordionContent
                          key={d + index}
